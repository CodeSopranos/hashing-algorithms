...
void getPerformanceString() {
    ...
    duration = duration_cast<microseconds>(stop - start).count();
    cout << "Average delete time: " << duration / testSize << " microsec" << endl;
    cout << "N attempts: " << openHmap.getNumberOfCollisions() / (testSize * 3) << endl;
  ...
  /*      CoCoo Hash Map part          */
    ...
    CuckooHashMap<string, string> cuckooHmap(tableSize, vecSize);
    for (auto const &value : baseVec) {
...
void getPerformanceStringToFile()
{
  ...
  for (auto i=1; i<=ITERATIONS; ++i){
    ...
    strCuckooHashMap cuckooHMap(2 * (vecSize + testSize + 1), vecSize + testSize);
    for(auto const& value: baseVec){
      cuckooHMap.insert(value, value);
    }
    auxPerformanceFunc<strCuckooHashMap, string>(cuckooHMap, testVec, insertCocoo, searchCocoo, deleteCocoo, attemptsCocoo);
